...
def plotSingleResolution(tree,name,title,selection=[],verbose=False,nbins=50,just_gauss=False,time=False):
    ...
    tree.SetAlias("Hit_CE", "Hit_NCherenkov/24.37") #MeV
    tree.SetAlias("Hit_layer", "4 - Hit_iz") # correct
    tree.SetAlias("gain", "( (Hit_layer == 1) || (Hit_layer == 2)) ? 1 : ( (Hit_layer == 4) ? 6 : 4 )")
    tree.SetAlias("noise_per_layer", "( (Hit_layer == 1) || (Hit_layer == 2)) ? 24 : ( (Hit_layer == 4) ? 6 : 4 )") #to be done.... need pedestal run
    tree.SetAlias("noise_per_ch", "noise_per_layer * ( ((Hit_ix==3) && (Hit_iy==3)) ? 2 : 1)")

    energy_residual = "Sum$(Hit_CE * (1 + rng/sqrt(Hit_CE * 0.6)) * (Hit_CE > 73 / gain) + rng*noise_per_ch )/(EnergyTotal) - 1"

    tree.Draw(f"{energy_residual}>>resotemp(400, -2, 2)", "", "goff")

    resotemp = ROOT.gDirectory.Get("resotemp")
    m = resotemp.GetMean()
    s = resotemp.GetRMS()
    reso = ROOT.TH1F("reso","resolution",nbins,m-5*s,m+5*s)
    tree.Draw(f"{energy_residual}>> reso",sel, "goff")
